...
1. **§13 外交の双方向化**: TS コアで達成済み — 3 勢力分の敵対度メーター分離
   (人間/ブン・タ＝タ/苦魚族。`buntaHostility`/`kugyoHostility`)、朝貢
   (`tributeCaptive` = 捕虜返還で大きく低下)、常時の業 (部族の自然悪化・苦魚最速。
   人間はドリフトなし = 中立ルート保護 §14.5.7)、襲撃勢力の重み付き抽選
   (`pickRaidFaction`) と max 敵対度での間隔決定。残り: Godot 版への移植と
   貢ぎ物資源 (§7/§14.5.8) を対価とする朝貢の拡張。
