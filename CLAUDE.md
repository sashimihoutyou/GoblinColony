...
こちらは TS コアと違い**本物のグリッドマップ + A* パスファインディング + 昼夜 + 勝敗判定**を持つ。
タイムスケールは 1 tick = 1 実秒 × `ticks_per_day=60` = 1 日 60 秒（Web 版ダッシュボードと統一）。
`params.gd` が日次レートを per-tick へ変換する（KI-02。per-tick 定数を素で書かない）。
検証はヘッドレスで:
