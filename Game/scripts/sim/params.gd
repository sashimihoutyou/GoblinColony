...
var day_ticks: int = 192         # 昼の tick 数 (= 日の 8 割。_init で再計算)
...
#  - 睡眠: 夜 (日の後ろ 2 割 = 0.2 日) に巣全体で就寝する (夜トリガー §5)。ただし
#    ゲージの減少 (と HP 回復) は寝床 (NEST) に到着してから始まる。移動速度を
#    225 タイル/日へ上げたため巣内の徒歩は短く (≈ 0.05〜0.1 日)、夜が短くても
#    解消 4.5/日 で必要量を賄える。なお sleep_latch は sleepiness≤sleep_off まで
#    保持されるので、夜明けまでに抜けきらない個体は朝へずれ込んで眠り続ける
#    (= 総睡眠量は保たれる)。昼の疲労発火 (sleep_on=0.8) は予備経路。
...
var mating_duration_ticks: int         # 寝床での性行為の所要 (0.3 日を変換。完了で妊娠成立)
...
# 成体 225 タイル/日 = 2.5 タイル/実秒 (1 日 90 秒)。巣の端から端 (56 タイル) を約 22 秒。
# (1 日を 90 秒へ延ばした際に据え置きだった 150 を、日内の歩行が食事・睡眠・遠征を
#  食い過ぎないよう「2.5 タイル/実秒」基準へ引き上げ。enemy_move は据え置き = 防衛猶予)。
...
func _init() -> void:
	...
	mating_duration_ticks = maxi(1, int(0.3 * tpd))
