...
# §11.5/A4: _field_tiles を巣口からの距離で分割した候補 (近い/遠い出現物の抽選用)。
# どちらかが空なら _step_field 側で _field_tiles 全体へフォールバックする。
var _field_tiles_near: Array = []  # 巣口に近い候補 (distance=0)
var _field_tiles_far: Array = []   # 巣口から遠い・マップ縁寄りの候補 (distance=1)
...
func _step_field() -> void:
	...
	# A4: 距離 (近い/遠い) はタイル位置から決定的に導出する (追加 RNG 不要 = FORAGE
	# ベースラインの RNG 消費順序 (湧き判定→タイル抽選→収量) を一切変えない)。
	f.distance = 1 if _field_tiles_far.has(p) else 0
	# A4: 種別の重み付き抽選は既存ロールの後に追加する 1 回 (新規ロールは既存の後/
	# 独立ブロックの規約)。FORAGE が外れたときのみ通常と異なる挙動になる。
	f.kind = _roll_field_kind()
	if f.kind == FieldResource.Kind.WANDERER:
		# WANDERER の出身部族をスポーン時に 1 回だけ決定 (以降は読み取りのみ)。
		f.tribe = "bunta" if rng.next_float() < 0.5 else "kugyo"
	field_resources.append(f)
	_event({"t": "field_spawn", "id": f.id, "x": f.x, "y": f.y, "amount": f.amount,
		"kind": f.kind, "distance": f.distance})
...
## A4: 出現物の種別を重み付き抽選する (FieldResource.Kind 0..6、合計 1.0)。
## RNG 消費は 1 float のみ (累積和に対する線形探索 = 決定的)。
func _roll_field_kind() -> int:
	var r := rng.next_float()
	var cum := 0.0
	for k in range(params.field_kind_weights.size()):
		cum += params.field_kind_weights[k]
		if r < cum:
			return k
	return FieldResource.Kind.FORAGE
	...
	for y in range(map.height):
		for x in range(map.width):
			...
			if map.get_tile(x, y) == TileMapData.TileType.EXTERIOR \
				...
				for gp in map.gates:
					var gd: int = maxi(abs(x - gp.x), abs(y - gp.y))
					if gd <= 3:
						near_gate = true
					min_gate_dist = mini(min_gate_dist, gd)
				if not near_gate:
					_field_tiles.append(Vector2i(x, y))
					# A4: 巣口から十分離れたタイルは「遠い」候補 (マップ縁寄り)。
					# それ以外は「近い」候補。どちらも空なら _field_tiles へ
					# フォールバックする (_step_field 側)。
					if min_gate_dist >= params.field_far_min_gate_dist:
						_field_tiles_far.append(Vector2i(x, y))
					else:
						_field_tiles_near.append(Vector2i(x, y))
