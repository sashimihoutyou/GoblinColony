...
- 食料経済は収支再校正済み（KI-26 / `food_economy_spec.md`）。食料は一食=1.0 単位
  ...
  減らない）。在庫0かつ空腹限界の個体は HP が削れて餓死する（§2.5）。「在庫が無く眠気も
  限界なら睡眠を優先」の救済則は残しているが、飢餓中は睡眠中の HP 自然回復も止まる
  ため、睡眠に逃げても餓死は進行する（不眠の飢餓ループ対策としての意味のみ残る）
...
- **不眠の飢餓ループ**: 在庫 0 で空腹ラッチが永久に立ち、睡眠 (= 唯一の HP 回復) を
  ...
  群れが慢性疲弊する。「在庫が無く眠気も限界なら睡眠優先」の救済則で解消。
