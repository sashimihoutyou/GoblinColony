...
このファイルは、Codex（Codex.ai/code）がこのリポジトリで作業する際の指針です。
...
- **Godot 第二期の実装状況は `backlog.md` が一次情報、設計教訓は `known_issues_world.md`
  KI-27〜29 を参照。** タイル改変時は床キャッシュ再構築が必須（KI-27）、autosave は
  テキスト精度の限界ゆえ「ロード後決定」を保証（KI-28）、防衛/各新力学の実数は D1 一括調整（KI-29）。

### ⚠ サブエージェント運用（worktree の分岐ベース検証）

`isolation:worktree` で起動したサブエージェントの作業ブランチは、**ローカル HEAD でなく
古いリモート ref から分岐することがある**（B5 で実際に PR #23 = A2 時点から分岐し、A3 を
再実装・A4 を欠いた成果物になった）。マージ前に必ず:

```
git merge-base HEAD <worktree-branch>   # 期待: 現 HEAD かその直近祖先
```

で分岐ベースを確認する。ズレていたら成果物を**そのままマージせず**、純粋な差分だけを
現 HEAD へ手で再適用する（重複実装・機能巻き戻しを防ぐ）。また各サブエージェントの
成果は「全スイート緑」を**メイン側で再検証**してから統合する（セッション上限での途中
停止・空コミットもあるため完了報告を鵜呑みにしない）。
